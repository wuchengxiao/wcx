(function() {
    ...
    function formatPastedLog(raw) {
        ...
        // 新增：将匹配"20xx-xx-xx xx:xx:xx.xxx"时间开头的片段前插入换行，保证每条日志独立成行
        // 支持匹配格式如2026-09-07 06:38:28.322开头的日志内容
        text = text.replace(/(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\.\d{3})/g, '\n$1');

        // 新增：严格匹配带前后空格的" at "字符串，在其前插入换行，保证堆栈轨迹单独成行
        text = text.replace(/\sat\s/g, '\n at ');

        // 最后过滤掉连续多余的空行，避免生成大量空白行
        text = text.replace(/\n{2,}/g, '\n').trim();
    ...
    function setView(mode) {
        ...
        if (mode === 'render') {
            ...
            btnToggleView.textContent = '编辑模式';
            renderLines();
        } else {
            editor.style.display = 'block';
            renderedView.style.display = 'none';
            btnToggleView.textContent = '显示为 HTML';
        }
    }
    ...
    function renderLines() {
        ...
        const regs = kws.map(kw => {
            ...
            return {
                ...
                color: kw.c || kw.color || kw.c || '#ffff00',
                raw: text
            };
        }
        );
    ...
    function highlightLine(lineNo) {
        ...
        if (el) {
            ...
            setTimeout( () => el.classList.remove('match'), 800)
        }
    }

    function addKeyword(text, color) {
        if (!text || !text.trim())
            return;
        keywords.push({
            k: text.trim(),
    ...
    function refreshKeywordList() {
        ...
        keywords.forEach( (kw, idx) => {
            ...
            sw.className = 'swatch';
            sw.style.background = kw.c;
            const span = document.createElement('span');
            span.textContent = kw.k;
    ...
    function gotoMatch(idx) {
        ...
        const node = renderedView.querySelector(`.line[data-line='${item.line}']`);
        if (node) {
            node.scrollIntoView({
                block: 'center'
            });
            node.classList.add('match');
            setTimeout( () => node.classList.remove('match'), 800);
            showStatus(`第 ${currentMatch + 1} / ${matches.length} 条，行 ${item.line}`)
        }
    }
    ...
    btnToggleView.addEventListener('click', () => {
        setView(viewMode === 'edit' ? 'render' : 'edit');
    }
    );

    btnAddKw.addEventListener('click', () => {
        addKeyword(newKw.value, newColor.value);
        newKw.value = '';
    }
    );
    ...
    btnAddFromSelection.addEventListener('click', () => {
        ...
        if (viewMode === 'edit') {
            ...
            sel = editor.value.substring(s, e);
        } else {
    ...
    btnHideToolbar.addEventListener('click', () => {
        ...
        btnShowToolbar.style.display = 'block';
    }
    );
    ...
    btnNext.addEventListener('click', () => {
        if (matches.length)
            gotoMatch((currentMatch + 1) % matches.length);
    }
    );
    ...
    btnGoto.addEventListener('click', () => {
        ...
        if (!n)
            return;
        const node = renderedView.querySelector(`.line[data-line='${n}']`);
        if (node) {
            node.scrollIntoView({
                block: 'center'
            });
