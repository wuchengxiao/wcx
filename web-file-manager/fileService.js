...
function createFile(name, content = '') {
    ...
    // 检查重名
    const isDuplicate = files.some(f => f.name.toLowerCase() === trimmedName.toLowerCase());
    if (isDuplicate) {
        return { success: false, message: `文件名 "${trimmedName}" 已存在，请重命名后创建。` };
    }
...
async function downloadFiles(ids) {
    ...
    try {
        ...
        return { success: true, message: `已成功打包 ${targets.length} 个文件，并触发 ZIP 归档包下载。` };
    } catch (err) {
