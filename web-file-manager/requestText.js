...
function getApiToken() {
    ...
    if (!cfg) return '';
    if (cfg.token && cfg.token !== '__ENCODED__') {
        return cfg.token;
    }
    if (cfg.encodedToken) {
        return decodeToken(cfg.encodedToken);
    }
    return '';
}
...
window.globalTextApiConfig = {
    ...
    token: '__ENCODED__',
    encodedToken: 'NDk0ZDgwMDA1MGYwNDI2ODk1ZWRhODVmOTZhNGMyOWUuMGlGV1hMOGdvY01KMERxWQ==',
    temperature: 1.0,
    web_search: false,
    stream: true,
    timeoutMs: 120000,
...
async function* requestTextByMessages({ messages, token, url, tools, tool_choice }) {
    ...
    const effectiveToken = token || getApiToken();
    if (!effectiveToken) throw new Error('未检测到API密钥');
    const requestBody = {
        model: cfg.model,
        messages,
